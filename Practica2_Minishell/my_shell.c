...
//Recorrido de lista de argumentos buscando un token '>', seguido de otro 
//token que será un nombre de fichero
int is_output_redirection(char **args){
    
    int i=0;
    int condicion=0;
    int fd;
    //Revisamos mientras existan tokens
    while (args[i]!=NULL && condicion==0)
    {
        //encontrado un token '>'
        if(args[i][0] == '>'){
            
            //seguido de otro token que sería el nombre del fichero        
            if (args[i+1])
            {
                //gestionamos la condición booleana para salir del bucle
                //ya que se ha encontrado el token
                condicion = 1;
                
                //se trata de un redireccionamiento
                //substituimos '>' por NULL
                args[i]=NULL;

                //open del fichero
                fd=open(args[i+1], O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR |S_IWUSR);
                //salta en caso de error con el mismo.
                if(fd==-1){
                    perror("Error");
                    return (-1);
                }

                //descriptor hará referencia al fichero en fd
                dup2(fd,1);
                //liberamos
                close(fd);
